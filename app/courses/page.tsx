      ...
      <div className="max-w-7xl mx-auto">
        {!isAdmin && user && !completion.isComplete && (
          <div className="mb-6 bg-red-500/10 border border-red-500/20 p-6 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <Link href="/profile" className="px-6 py-3 bg-red-500 text-white rounded-xl font-bold uppercase tracking-widest text-[10px] hover:bg-red-600 transition-colors whitespace-nowrap">
              Complete Profile
            </Link>
          </div>
        )}
              ...
              {filtered.map((c, i) => (
                ...
                >
                  <div className="relative h-56 overflow-hidden rounded-t-[2rem] md:rounded-t-[3rem]">
                    <img 
                      src={c.imageUrl || "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auhref=format&fit=crop&q=80&w=800"} 
                      alt={c.title}
